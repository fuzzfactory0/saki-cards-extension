...
**Long story:** The extension needs to communicate with a remote server which handles the rooms and the game flow, hosted in the Oracle cloud. Now, acquiring and setting up SSL certificates and a proper domain is a pain (and costs money), so the extension communicates with the server through the regular HTTP protocol instead of HTTPS. Since autotable.riichi.moe is a secure website, Chrome doesn't like the fact that a secure website which was loaded through HTTPS is sending random HTTP requests, so the browser blocks those "mixed protocol requests" automatically by default. By allowing insecure content to reach that particular website, you're telling Chrome to ignore the fact that the extension is using a different protocol, and this is essential for the game to work.
